...
class GoogleAPI(object):
    ...
    def delete_video(self, data):
        return requests.delete(
            self.base_url + "?id=" + data["id"], headers=self._headers
        )
    ...
    def refresh_access_token(self):
        ...
        resp = requests.post("https://accounts.google.com/o/oauth2/token", data=params)
...
def uploadToYouTube(video):
    ...
    video.video_url = "https://www.youtube.com/watch?v=%s" % video_id
...
def updateYouTubePermissions(video):
    ...
    if is_public:
        if video.youtube_data["status"]["privacyStatus"] != "public":
            api.edit_video(
                video.youtube_data, video.Title(), video.Description(), "public"
            )
            video.youtube_data["status"]["privacyStatus"] = "public"
    else:
        if video.youtube_data["status"]["privacyStatus"] != "unlisted":
            api.edit_video(
                video.youtube_data, video.Title(), video.Description(), "unlisted"
            )

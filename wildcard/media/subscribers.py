...
def video_state_changed(video, event):
    if getattr(video, "upload_video_to_youtube", False) and getattr(
...
def retrieveThumbImage(video):
    ...
    try:
        ...
        api.portal.show_message(
            message=error_msg, request=video.REQUEST, type="warning"
        )
        return
    ...
    if not res.ok:
        if res.status_code == 404:
            ...
            logger.error(
                'Unable to retrieve thumbnail from "%s". Error: %s'
                % (url, res.status_code)
            )
            api.portal.show_message(
                message=error_msg, request=video.REQUEST, type="warning"
        ...
        return
    video.image = NamedBlobImage(res.raw.data, filename="%s.jpg" % video_id)
    api.portal.show_message(
        message=_(

(function () {
   ...
   */
  define("pat-logger", ["logging"], function (logging) {
    var log = logging.getLogger("patterns");
    return log;
  });

  define("pat-utils", ["jquery"], function ($) {
    ...
    function rebaseURL(base, url) {
      if (url.indexOf("://") !== -1 || url[0] === "/") return url;
      ...
      this.each(function () {
        ...
        if ($ancestor.length && $ancestor.css("position") !== "static") {
          ...
          var childMarginEdgeLeft =
            $child.offset().left - parseInt($child.css("marginLeft"), 10);
          var childMarginEdgeTop =
            $child.offset().top - parseInt($child.css("marginTop"), 10);
          var ancestorPaddingEdgeLeft =
            $ancestor.offset().left +
  ...
  define("pat-registry", [
    ...
    $(document).on(
      "patterns-injected.patterns",
      function registry_onInject(ev, config, trigger_el, injected_el) {
        if (
          injected_el.nodeType !== TEXT_NODE &&
          injected_el !== COMMENT_NODE
        ) {
          registry.scan(injected_el, null, {
            type: "injection",
            element: trigger_el,
          });
  ...
  // vim: sw=4 expandtab
  define("pat-mockup-parser", ["jquery"], function ($) {
    "use strict";
  ...
  define("mockup-patterns-base", ["jquery", "pat-base"], function ($, Base) {
    "use strict";
...
*/
  ...
  mejs.HtmlMediaElementShim = {
    ...
    createErrorMessage: function (playback, options, poster) {
      ...
      htmlMediaElement.parentNode.insertBefore(
        errorContainer,
        htmlMediaElement
      );
      htmlMediaElement.style.display = "none";
...
*/
  ...
  (function ($) {
    ...
    $.extend(MediaElementPlayer.prototype, {
      ...
      updateCurrent: function () {
        ...
        if (t.currenttime) {
          t.currenttime.html(
            mejs.Utility.secondsToTimeCode(t.media.currentTime, t.options)
          );
        }
      },
            ...
            if (t.isInIframe) {
              ...
              setTimeout(function checkFullscreen() {
                if (t.isNativeFullScreen) {
                  ...
                  if (
                    Math.abs(screenWidth - windowWidth) >
  ...
  (function ($) {
    ...
    $.extend(MediaElementPlayer.prototype, {
      builduniversalgoogleanalytics: function (
        player,
        controls,
        layers,
        media

...
class AudioView(MediaView):
    def __call__(self):
        ...
        self.audio_url = "%sIAudio.audio_file/@@stream" % (base_wurl)
...
class GlobalSettingsForm(group.GroupForm, form.EditForm):
    ...
    label = _("Media Settings")
    description = _(
        "description_media_global_settings_form",
...
class Utils(MediaView):
    ...
    def videos(self):
        ...
        for _type, fieldname in types:
            ...
            if file:
                videos.append(
                    {"type": _type, "url": self.base_furl + fieldname + "/@@stream"}
                )
        return videos
